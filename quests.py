...
QUESTS = [
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "world_flavor": "Kazdy wojownik potrzebuje dobrego ekwipunku.",
        ...
        "boss": {
            "title": "BOSS FIGHT 0.1 — Ekwipunek",
            "description": "Sprawdz czy Homebrew jest zainstalowany:",
            "steps": [
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "xp": 15,
        "theory": [
            {"cmd": "pwd",       "what": "Gdzie jestem? (Print Working Directory)",         "notes": "pwd -> /Users/zenon"},
            ...
            {"cmd": "cd ~",      "what": "Wroc do domu (/Users/zenon)",                    "notes": "~ to zawsze skrot do twojego home"},
        ...
        "boss": {
            "title": "BOSS FIGHT 1.1 — Nawigator",
...

    {
        ...
        "xp": 15,
        "theory": [
            {"cmd": "whoami",       "what": "Kim jestem? Wypisuje nazwe uzytkownika",      "notes": ""},
...

    {
        ...
        "boss": {
            "title": "BOSS FIGHT 1.3 — Skroty",
            "description": "Przetestuj skroty klawiszowe w terminalu:",
            "steps": [
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "boss": {
            "title": "BOSS FIGHT 2.1 — Budowniczy i Niszczyciel",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "xp": 25,
        "theory": [
            {"cmd": "ls -la | less",            "what": "Ogromna lista → przewijalna",          "notes": "| przekazuje output dalej"},
...

    {
        ...
        "xp": 25,
        "theory": [
            {" cmd": ">",    "cmd": ">",   "what": "Zapisz output do pliku (NADPISUJE!)",  "notes": "Jesli plik istnieje — nadpisuje bez pytania"},
            {"cmd": ">>",   "what": "Dopisz do pliku",                                    "notes": "Bezpieczne — nie niszczy istniejacych danych"},
            {"cmd": "<",    "what": "Wczytaj z pliku jako input",                         "notes": "sort < lista.txt = to samo co sort lista.txt"},
            {"cmd": "2>",   "what": "Przekieruj bledy (stderr)",                          "notes": "/dev/null = wirtualny kosz, dane znikaja"},
...

    {
        ...
        "title": "Potezne narzedzia",
        "xp": 30,
        "theory": [
            {"cmd": "sort",             "what": "Sortuj alfabetycznie",                  "notes": "-n = numerycznie  -r = odwrotnie  -rn = od najwiekszego"},
            {"cmd": "uniq",             "what": "Usun sasiadujace duplikaty",             "notes": "ZAWSZE po sort! uniq nie widzi nieniasiadujacych duplikatow"},
        ...
        "boss": {
            ...
            "steps": [
                ...
                {"type": "shell", "cmd": f"cut -d' ' -f1 {QP}/access.log",
                 "hint": "1. Pokaz tylko adresy IP (kolumna 1, separator spacja)"},
                {"type": "shell",
                 "cmd": f"cut -d' ' -f1 {QP}/access.log | sort | uniq -c | sort -rn",
                 "hint": "2. Ktore IP ma najwiecej requestow?"},
                {"type": "shell", "cmd": f"grep -E ' [45][0-9]{{2}}$' {QP}/access.log",
                 "hint": "3. Wszystkie bledy (kody 4xx i 5xx)"},
                {"type": "shell", "cmd": f"rm -r {QP}",
                 "hint": "posprzataj"},
            ],
        },
...

    {
        ...
        "boss": {
            "title": "BOSS FIGHT 4.2 — Vim",
            "description": "Uruchom wbudowany tutorial Vima (ok. 30 min):",
            "steps": [
                {"type": "info",
                ...
                {"type": "confirm",
                 "prompt": "Czy ukonczyłes vimtutor i wiesz jak wychodzic z Vima (:wq)?"},
            ],
        },
    },
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "boss": {
            "title": "BOSS FIGHT 5.1 — Straznik Uprawnien",
            "description": "Cwicz zarzadzanie uprawnieniami:",
            "steps": [
...

    {
        ...
        "xp": 20,
        "theory": [
            {"cmd": "echo $HOME",        "what": "Twoj katalog domowy",                  "notes": ""},
            {"cmd": "echo $PATH",        "what": "Gdzie system szuka programow",         "notes": "Lista katalogow oddzielona :"},
            {"cmd": "echo $USER",        "what": "Twoja nazwa uzytkownika",              "notes": ""},
        ...
        "boss": {
            "title": "BOSS FIGHT 5.2 — Konfiguracja",
            "description": "Zbadaj zmienne i dodaj alias:",
            "steps": [
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "xp": 25,
        "theory": [
            {"cmd": "ps aux",         "what": "Lista wszystkich procesow",              "notes": "a=wszyscy uzytkownicy  u=czytelny format  x=bez terminala"},
            {"cmd": "top -o cpu",     "what": "Monitor na zywo sortowany po CPU",      "notes": "-o = order by. q = wyjscie"},
            {"cmd": "komenda &",      "what": "Uruchom w tle",                         "notes": "& na koncu — shell wraca do promptu od razu"},
            {"cmd": "Ctrl+Z",         "what": "Zawies biezacy proces",                 "notes": "Zatrzymaj, nie zabij"},
            {"cmd": "bg / fg",        "what": "Wznow w tle / przywroc na plerwszy plan","notes": ""},
            {"cmd": "jobs -l",        "what": "Lista procesow w tle z PID",            "notes": "-l = long (pokaz PID)"},
            {"cmd": "kill PID",       "what": "Grzeczna prosba o zakonczenie",         "notes": "Wysyla SIGTERM"},
            {"cmd": "kill -9 PID",    "what": "Natychmiastowe zabicie",                "notes": "-9 = SIGKILL. Ostatecznosc!"},
            {"cmd": "lsof -i :8080",  "what": "Co uzywa portu 8080?",                 "notes": "Gdy 'port is already in use'"},
        ],
        "boss": {
            "title": "BOSS FIGHT 6.1 — Pogromca Procesow",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "boss": {
            "title": "BOSS FIGHT 7.1 — Pierwszy Skrypt",
            ...
            "steps": [
                ...
                {"type": "shell", "cmd": f"{QP}/hello.sh Zenon",
                 "hint": "uruchom z argumentem — zobaczysz 'Witaj, Zenon!'"},
                {"type": "shell", "cmd": f"rm -r {QP}",
                 "hint": "posprzataj"},
            ],
        },
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "title": "Siec i diagnostyka",
        "xp": 25,
        "theory": [
            {"cmd": "ping google.com",     "what": "Sprawdz polaczenie z internetem",    "notes": "Ctrl+C przerwij  -c 4 = wyslij tylko 4 pakiety"},
            ...
            {"cmd": "scp plik u@s:/path",  "what": "Kopiuj plik na serwer",             "notes": "scp = secure copy"},
            {"cmd": "ssh-keygen -t ed25519","what": "Generuj klucz SSH",                "notes": "ed25519 = nowoczesny algorytm. Lepszy niz rsa"},
        ],
        "boss": {
            "title": "BOSS FIGHT 8.1 — Sieciowy Mag",
            "description": "Cwicz komendy sieciowe:",
            "steps": [
                {"type": "shell", "cmd": "curl -s ifconfig.me",
                 "hint": "Pokaz swoje publiczne IP"},
                ...
                {"type": "shell", "cmd": "curl -s https://api.github.com | head -5",
                 "hint": "Pobierz dane z publicznego API GitHub"},
            ],
        },
    },
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "boss": {
            "title": "BOSS FIGHT 9.1 — Git Basics",
            "description": "Zrob pelny cykl: init, add, commit:",
            "steps": [
                {"type": "shell", "cmd": f"mkdir -p {QP} && cd {QP}",
                 "hint": "stworz i wejdz do folderu projektu"},
                {"type": "shell", "cmd": f"git init {QP}",
                 "hint": "zainicjuj repo w folderze quest-project"},
                {"type": "shell", "cmd": f"echo 'Hello Git' > {QP}/plik.txt",
...

    {
        ...
        "theory": [
            ...
            {"cmd": "git merge feature",   "what": "Polacz branch z aktualnym",         "notes": "Badzac na main: git merge feature"},
            {"cmd": "git branch -d feat",  "what": "Usun branch",                      "notes": "-d = tylko jesli juz zmergowany. -D = na sile"},
        ],
        "boss": {
            "title": "BOSS FIGHT 9.2 — Branche",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "theory": [
            ...
            {"cmd": "awk -F: '{print $1}' /etc/passwd",  "what": "Wypisz pierwsza kolumne z : jako separatorem", "notes": "-F: = separator :"},
        ],
        "boss": {
            "title": "FINAL BOSS — Arcymistrz",
            "description": "Pokonaj ostateczne wyzwanie brace expansion:",
            "steps": [
